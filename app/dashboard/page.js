...
export default function Dashboard() {
  ...
  // Read the emergency filter from localStorage on mount
  useEffect(() => {
    const filter = localStorage.getItem("emergencyFilter") || "";
  ...
  // Listen for requests from Realtime Database
  useEffect(() => {
    if (!adminInfo) return;
  ...
  // Delete a request from the Realtime Database
  ...
  // Filter requests based on emergency filter (case-insensitive)
  const filteredRequests = emergencyFilter
    ? requests.filter(
      request =>
        request.requestType &&
        request.requestType.toLowerCase() === emergencyFilter.toLowerCase()
    )
    : requests;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100">
        ...
        <div className="container mx-auto flex justify-between items-center flex-wrap"> {/* Added flex-wrap */}
          <div className="flex items-center gap-3 mb-2 md:mb-0"> {/* Added margin-bottom for small screens */}
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-base md:text-lg text-gray-300">Welcome, {adminInfo?.username}</span>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRequests.length > 0 ? (
              filteredRequests.map((request) => {
                // Lookup the requester information from the users list
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col mb-3 flex-grow">
                      ...
                      {/* Display emergency type right below the name */}
                      {request.requestType && (
